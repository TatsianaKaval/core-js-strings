...
function removeFirstOccurrences(str, value) {
  const substrInd = str.indexOf(value);
  if (substrInd === -1) return str;
  return str.slice(0, substrInd) + str.slice(substrInd + value.length);
}
...
function removeLastOccurrences(str, value) {
  const substrInd = str.lastIndexOf(value);
  if (substrInd === -1) return str;
  return str.slice(0, substrInd) + str.slice(substrInd + value.length);
}
...
function isPalindrome(str) {
  const cleanStr = str.toLowerCase().replaceAll(/[^a-z0-9]/g, '');
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < wordArray.length; i += 1) {
    if (wordArray[i].length > searchWord.length) {
...
function extractNameFromTemplate(value) {
  return value.split(', ')[1].slice(0, -1).toString();
}
